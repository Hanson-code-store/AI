...
void ExitProgram(const char *message) {
    ...
    output->score = Points;
    output->node_sec = (output->expanded)/(output->time);
    printOutput(output);
...
void printOutput(output_t* o){
    ...
    if(o->prop == 1){
        fprintf(ofp,"Propagation = Avg\n");
    }else{
        fprintf(ofp,"Propagation = MAX\n");
    }
    
    fprintf(ofp,"Budget = %d\n",o->budget);
    fprintf(ofp,"MaxDepth = %d\n",o->max_depth);
    fprintf(ofp,"TotalGenerated = %d\n",o->generated);
    fprintf(ofp,"TotalExpanded = %d\n",o->expanded);
    fprintf(ofp,"Expanded/Second = %f\n",o->node_sec);
    fprintf(ofp,"Time = %f\n",o->time);
    fprintf(ofp,"MaxValue = %d\n",o->max_board);
    fprintf(ofp,"Score = %d\n",o->score);
...
void MainLoop() {
    ...
    do {
        ...
        if(ai_run){
            ...
            move_t selected_move = get_next_move(current_state, 
                                                 budget, propagation, ai_stats,prev,p_prev,output);
            gettimeofday(&stop, NULL);
            
            float elapsed_s = (stop.tv_sec - start.tv_sec) *1.0 ;
            elapsed_s += (stop.tv_usec - start.tv_usec) / 1000000.0;
            output->time += elapsed_s; 
